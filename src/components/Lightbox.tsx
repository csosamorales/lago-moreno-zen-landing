...
export default function Lightbox({ slides, index, onClose, onNavigate }: LightboxProps) {
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button onClick={onClose} className="text-white hover:text-gold text-2xl leading-none" aria-label="Cerrar">×</button>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-16 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-full max-w-4xl h-[60vh]">
          <PhotoPlaceholder className="w-full h-full" label={slides[index]?.label ?? ''} />
        </div>
        <button
      ...
      <div className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
        <p className="text-white/60 text-xs text-center mb-3">{slides[index]?.label ?? ''}</p>
        <div className="flex gap-2 justify-center overflow-x-auto">
          {slides.map((s, i) => (
            ...
            >
              <PhotoPlaceholder className="w-full h-full" label="" />
            </button>
          ))}
        </div>
      </div>
